...
class _ManifestEntry(BaseModel):
    ...
    def validate_policy_evidence(self) -> "_ManifestEntry":
        policy = (
...
class _RecoveryManifest(BaseModel):
    ...
    entries: list[_ManifestEntry] = Field(min_length=9, max_length=9)
    admin_compensation: _AdminAllocation

    @model_validator(mode="after")
    def validate_one_shot_allocation(self) -> "_RecoveryManifest":
...
class LegacyReferralRecoveryAuthorizer:
    ...
    def _load_manifest(self) -> _RecoveryManifest:
        ...
        try:
            manifest = _RecoveryManifest.model_validate(parsed)
